...
export default function FromBank() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    setWalletAddress("");
    setLoading(false);
    setTimeout(() => router.push("/Exchanges/FromBank/Bank"));
  };
          ...
          <form className="p-8 relative" onSubmit={handleFormSubmit}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                {walletId && (
                  <p
                    className={`text-xs mt-1 ${isWalletID(walletId) ? "text-green-600" : "text-orange-600"}`}
                  >
                    {isWalletID(walletId)
                      ? "Valid wallet ID"
                      : "Wallet ID should be 26+ chars"}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {walletAddress && (
                  <p
                    className={`text-xs mt-1 ${isWalletAddress(walletAddress) ? "text-green-600" : "text-orange-600"}`}
                  >
                    {isWalletAddress(walletAddress)
                      ? "Valid wallet address"
                      : "Wallet address should be 26+ chars"}
                  </p>
                )}
              </div>
